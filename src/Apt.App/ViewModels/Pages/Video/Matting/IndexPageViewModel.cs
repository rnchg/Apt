...
namespace Apt.App.ViewModels.Pages.Video.Matting
{
    public partial class IndexPageViewModel : CommonViewModel
    {
        ...
        public override void InitializeViewModel()
        {
            ...
            ModeSource =
            [
                new ComBoBoxItem<string>() {  Text = Language.Instance["Video.Matting.ModeStandard"], Value = "Standard" }
            ];

            FileGridSwitchItem = FileSwitch.Input;

            AddMessage(MessageType.Success, Language.Instance["Video.Matting.Help"]);
        ...
        public override async Task Start()
        {
            try
            {
                ...
                {
                    throw new Exception(Language.Instance["Video.Matting.InputError"]);
                }
                if (!Directory.Exists(Output))
                {
                    throw new Exception(Language.Instance["Video.Matting.OutputError"]);
                }
                var inputFiles = FileGridTableList.Select(e => e.FullName).ToArray();
                if (inputFiles.Length == 0)
                {
                    throw new Exception(Language.Instance["Video.Matting.FileError"]);
                }

                await _indexService.StartAsync(Input, Output, inputFiles, Provider, Mode);

                SnackbarService.ShowSnackbarSuccess(Language.Instance["Video.Matting.ProcessEnd"]);
