...
using General.Apt.Service.Services.Pages.Video.Matting;
using General.Apt.Service.Utility;
using Microsoft.Win32;
using System.Windows.Documents;
using Wpf.Ui.Controls;

namespace General.Apt.App.ViewModels.Pages.Video.Matting
{
    public partial class IndexViewModel : ObservableValidator, INavigationAware
    {
        ...
        public void InitializeViewModel()
        {
            ...
            {
                new ComBoBoxItem<string>() { Text = Language.Instance["VideoMattingIndexPageInputSortName"], Value = "Name" },
                new ComBoBoxItem<string>() { Text = Language.Instance["VideoMattingIndexPageInputSortLastWriteTime"], Value = "LastWriteTime" },
                new ComBoBoxItem<string>() { Text = Language.Instance["VideoMattingIndexPageInputSortLength"], Value = "Length" }
            };
            SortRuleSource = new ObservableCollection<ComBoBoxItem<string>>()
            {
                new ComBoBoxItem<string>() { Text = Language.Instance["VideoMattingIndexPageInputSortRuleAsc"], Value = "Asc" },
                new ComBoBoxItem<string>() { Text = Language.Instance["VideoMattingIndexPageInputSortRuleDesc"], Value = "Desc" }
            };
        ...
        public async Task Start()
        {
            try
            {
                ...
                Message.ShowSnackbarSuccess(Language.Instance["VideoMattingIndexPageOperationCompleted"]);
