...
using General.Apt.Service.Services.Pages.Image.Matting;
using General.Apt.Service.Utility;
using Microsoft.Win32;
using System.Windows.Documents;
using Wpf.Ui.Controls;

namespace General.Apt.App.ViewModels.Pages.Image.Matting
{
    public partial class IndexViewModel : ObservableValidator, INavigationAware
    {
        ...
        public void InitializeViewModel()
        {
            ...
            {
                new ComBoBoxItem<string>() { Text = Language.Instance["ImageMattingIndexPageInputSortName"], Value = "Name" },
                new ComBoBoxItem<string>() { Text = Language.Instance["ImageMattingIndexPageInputSortLastWriteTime"], Value = "LastWriteTime" },
                new ComBoBoxItem<string>() { Text = Language.Instance["ImageMattingIndexPageInputSortLength"], Value = "Length" }
            };
            SortRuleSource = new ObservableCollection<ComBoBoxItem<string>>()
            {
                new ComBoBoxItem<string>() { Text = Language.Instance["ImageMattingIndexPageInputSortRuleAsc"], Value = "Asc" },
                new ComBoBoxItem<string>() { Text = Language.Instance["ImageMattingIndexPageInputSortRuleDesc"], Value = "Desc" }
            };
        ...
        public async Task Start()
        {
            try
            {
                ...
                Message.ShowSnackbarSuccess(Language.Instance["ImageMattingIndexPageOperationCompleted"]);
